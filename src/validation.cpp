...
#if defined(NDEBUG)
# error "Kepler Core cannot be compiled without assertions."
#endif
                  ...
                  CCoinsViewCache& view, const CChainParams& chainparams, bool fJustCheck = false)
{
    ...
    // Start enforcing BIP68 (sequence locks) and BIP112 (CHECKSEQUENCEVERIFY) using versionbits logic. (CSV Fork)
    ...
    // BIP147, deployed using DEPLOYMENT_BIP147, removed. This is a good example of a BIP9 softfork deployment.
    ...
    // KPL : MODIFIED TO CHECK MASTERNODE PAYMENTS AND SUPERBLOCKS
    ...
    if (!IsBlockValueValid(block, pindex->nHeight, blockReward, strError)) {
        return state.DoS(0, error("ConnectBlock(KPL): %s", strError), REJECT_INVALID, "bad-cb-amount");
    }

    if (!IsBlockPayeeValid(*block.vtx[0], pindex->nHeight, blockReward)) {
        mapRejectedBlocks.insert(std::make_pair(block.GetHash(), GetTime()));
        return state.DoS(0, error("ConnectBlock(KPL): couldn't find masternode or superblock payments"),
    ...
    if (!IsInitialBlockDownload())
    {
        ...
        for (int i = 0; i < 100 && pindex != NULL; i++)
        {
            ...
            if (pindex->nVersion > VERSIONBITS_LAST_OLD_BLOCK_VERSION && ((pindex->nVersion & ~nExpectedVersion) & 0xFF0000FF) != 0){
                ++nUpgraded;
                //LogPrintf("expected 0x%08x, recieved 0x%08x at height %d \n", nExpectedVersion, pindex->nVersion, pindex->nHeight); // debug
            }   
            pindex = pindex->pprev;
        }
        if (nUpgraded > 0)
...
bool CheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, bool fCheckPOW, bool fCheckMerkleRoot)
{
    ...
    // KPL : CHECK TRANSACTIONS FOR INSTANTSEND
    ...
    if(sporkManager.IsSporkActive(SPORK_3_INSTANTSEND_BLOCK_FILTERING)) {
        ...
        LogPrintf("CheckBlock(KPL): spork is off, skipping transaction locking checks\n");
    }
...
bool ContextualCheckBlock(const CBlock& block, CValidationState& state, const Consensus::Params& consensusParams, const CBlockIndex* pindexPrev)
{
    ...
    // Start enforcing BIP113 (Median Time Past) using versionbits logic. (CSV Fork)
    int nLockTimeFlags = 0;
