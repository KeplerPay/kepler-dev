...
CMasternode::CollateralStatus CMasternode::CheckCollateral(const COutPoint& outpoint, const CPubKey& pubkey, int& nHeightRet)
{
    ...
    if(coin.out.nValue != Params().GetConsensus().nMasternodeCost * COIN) {
...
bool CMasternodeBroadcast::CheckOutpoint(int& nDos)
{
    ...
    if (err == COLLATERAL_INVALID_AMOUNT) {
        LogPrint("masternode", "CMasternodeBroadcast::CheckOutpoint -- Masternode UTXO should have %d KPL, masternode=%s\n", Params().GetConsensus().nMasternodeCost, outpoint.ToStringShort());
        nDos = 33;
        return false;
    }
